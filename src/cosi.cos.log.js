  ...
  consoleUpFilesLog: (upFileLogs, files, startTime, hex, obsClient) => {
    ...
    if (upFileLogs.length == files.length) {
      ...
      for (let index = 0; index < upFileLogs.length; index++) {
        let code = upFileLogs[index].item.statusCode;
        if (code > 200) {
          ERRORNum += 1;
          console.log(
            colors.bgRed(
              " ERROR " + upFileLogs[index].item.statusCode + " "
            ),
            colors.red(" " + upFileLogs[index].item.code)
          );
          console.log(
            colors.red(
                upFileLogs[index].item.stack
            )
          );
        } else if (code == "WARN") {
          WARNNum += 1;
      ...
      console.log(colors.green.bold("TencentCloud OBS Mission Over 🔆"));
      
    }
  },
  requestErrorLog: (error, obsClient) => {
    console.log(
      colors.bgRed(" TencentCloud OSS ERROR " + error.code + " "),
